...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	franticfusionsmod: {
		...
		onModifySpecies(species, target, source, effect) {
			...
			newSpecies.bst = newSpecies.baseStats.hp;
			for (const stat in newSpecies.baseStats) {
				newSpecies.baseStats[stat] = this.clampIntRange((newSpecies.baseStats[stat] + fusionSpecies.baseStats[stat]) / 2, 1, 255);
				newSpecies.bst += newSpecies.baseStats[stat];
			}
			return newSpecies;
		},
	...
	datamod: {
		...
		onSwitchIn(pokemon) {
			...
			if (this.format.name.includes('Frantic Fusions')) {
				const newSpecies = this.dex.deepClone(species);
				const fusionName = pokemon.set.name;
				if (!fusionName || fusionName === newSpecies.name) return;
				const fusionSpecies = this.dex.deepClone(this.dex.species.get(fusionName));
				newSpecies.bst = newSpecies.baseStats.hp;
				for (const stat in newSpecies.baseStats) {
